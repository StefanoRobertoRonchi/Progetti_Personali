In the following section is reported a Python implementation of the Stochastic Search Variable Selection (SSVS) described in  \textit{Variable Selection by Gibbs Sampling} (Robert McCulloch & Edward George,1993). 
...
$c_j$ is the ratio of heights between the slab (i.e., distribution when $\gamma_j = 1$) and spike (i.e., distribution when $\gamma_j = 0$) evaluated at 0 and can be interpreted as the \textbf{prior odds} that the $j^{th}$ regressor should be included or not. 
...
- (10,100)
- (10,500)
Therefore the choice of $\tau_j$ depends on the scale of the data making the algorithm \textbf{not} scale invariant.

The procedure can be a suitable alternative to OLS stepwise selection algorithm without requiring to compute model statistics (e.g., $R^2, AIC, BIC$) for a large subset of models and provides a measure of \textbf{uncertainty related to the variables within the model}. In credit risk can be used to develop \textbf{satellite model to link the Probability of Default to macroeconomic conditions} and make scenario stress test analysis.
...
$$
\gamma_j \in \{0,1\} \qquad  \gamma_j \sim Ber(p_j) \qquad (2)
$$
$$
\beta_j \mid \gamma_j \sim 
\mathcal{N}\!\left(
...
\right)
$$
In vector form: $\beta \mid \gamma \sim \mathcal{N}\!\left(0,\; D_{\gamma} R D_{\gamma}^{\top}\right)$. with  $D_{\gamma}$  defined as \[
D_{\Gamma} =
...
\end{cases}
\quad 
R = \sigma^2( X'X)^{-1}  \quad or \quad I
\]
...
\begin{itemize}
    \item $\beta \mid y, \gamma, \sigma^2$ \qquad (3)
    \item $\gamma \mid y, \sigma^2, \beta$ \qquad (4)
    \item $\sigma^2 \mid y, \beta, \gamma$ \qquad (5)
\end{itemize}
...
&\sim \mathcal{N}\!\left(\beta^*,\, K^{-1}\right),\\[4pt]
\beta^* 
&= K^{-1}\frac{X^{\top}y}{\sigma^2},\\[4pt]
K 
&= \frac{X^{\top}X}{\sigma^2} + \left(D_\gamma R D_\gamma^{\top}\right)^{-1}.
\end{align*}
\begin{align*}
\sigma^2 \mid y,\beta,\gamma &\sim \mathcal{IG}(a^*, b^*),\\[4pt]
a^* &= a_0 + \frac{n}{2},\\
b^* &= b_0 + \frac{1}{2}(y - X\beta)^{\top}(y - X\beta).
\end{align*}
\begin{align*}
P(\gamma_j = 1 \mid \beta, \gamma_{-j},\sigma^2) = P(\gamma_j = 1 \mid \beta, \gamma_{-j}) =  \frac{PDF_{\beta_j, \gamma_j=1}}{PDF_{\beta_j, \gamma_j=1} + PDF_{\beta_j, \gamma_j=0}}
\end{align*}
